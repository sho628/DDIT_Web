		...
		<tr>
			<td colspan="6">
				...
				<div id="searchUI" class="form-inline">
					<select name="prodLgu" id="prodLgu" class="form-control mr-3">
						<option value>상품분류</option>
						<c:forEach items="${lprodList }" var="lprod">
							<option value="${lprod['lprodGu'] }">${lprod.lprodNm }</option>
						</c:forEach>
					</select>
					<select name="prodBuyer" id="prodBuyer" class="form-control mr-3">
						<option value>거래처</option>
						<c:forEach items="${buyerList }" var="buyer">
							<option value="${buyer.buyerId }" class="${buyer.buyerLgu }">${buyer.buyerName }</option>
						</c:forEach>
					</select>
					<input type="text" name="prodName"  class="form-control mr-3" placeholder="상품명"/>
					<input type="button" id="searchBtn" value="검색" class="btn btn-primary mr-3" />
					<security:authorize url="/prod/prodInsert.do">
						<input type="button" class="linkBtn btn btn-success" 
