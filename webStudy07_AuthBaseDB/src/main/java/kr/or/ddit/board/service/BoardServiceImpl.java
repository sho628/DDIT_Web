...
//		encryptPassword(board);
	...
	public ServiceResult removeBoard(BoardVO board) {
		...
		if(authenticated instanceof BoardVO) {
			BoardVO saved = (BoardVO) authenticated;
			//===================계층형 구조상의 하위 글을 삭제하지 않고, 삭제여부 상태값만 변경하는 경우 활용.
//			List<AttatchVO> attatchList = saved.getAttatchList();
//			attatchDAO.deleteAttatches(board);
...
//				FileUtils.deleteQuietly(new File(saveFolder, tmp.getAttSavename()));
//			}
			//===================계층형 구조상의 하위 글들과 해당 글의 덧글 및 첨부파일까지 모두 지우는 경우 활용.
			Map<String, Object> pMap = new HashMap<>();
			pMap.put("boNo", board.getBoNo());
			boardDAO.deleteBoardUsingProcedure(pMap);
			log.info("{} 개의 게시글 삭제", pMap.get("rowcnt"));
			String[] saveNames = (String[]) pMap.get("saveNames");
			log.info("삭제대상 첨부파일 : {}", Arrays.toString(saveNames));
